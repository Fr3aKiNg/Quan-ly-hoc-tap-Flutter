...
class MyDetailCoursePage extends StatelessWidget {
  final String course;
  final String uid;
  MyDetailCoursePage({Key key, @required this.course, @required this.uid})
      : super(key: key);
  @override
...
class detailCourse extends StatefulWidget {
  final String course;
  final String uid;
  detailCourse({Key key, @required this.course, @required this.uid})
      : super(key: key);
  @override
...
class detailCourseState extends State<detailCourse> {
  ...
  double getAvg(List _score) {
    ...
    for (int i = 0; i < _heso.length; i++) {
      if (_heso[i] is String) _heso[i] = int.parse(_heso[i]);
    }
    for (int i = 0; i < _heso.length; i++) {
        ...
        setState(() {
          if (chosenYear == 0) {
            for (int i = 0; i < _nameCol.length; i++) {
              _scoreSes1.add(value.data["year1"]["HK1"]["ingredientScore"]
                  [CourseName][_nameCol[i]]);
              _scoreSes2.add(value.data["year1"]["HK2"]["ingredientScore"]
                  [CourseName][_nameCol[i]]);
            }
          } else if (chosenYear == 1) {
            for (int i = 0; i < _nameCol.length; i++) {
              _scoreSes1.add(value.data["year2"]["HK1"]["ingredientScore"]
                  [CourseName][_nameCol[i]]);
              _scoreSes2.add(value.data["year2"]["HK2"]["ingredientScore"]
                  [CourseName][_nameCol[i]]);
            }
          } else {
            for (int i = 0; i < _nameCol.length; i++) {
              _scoreSes1.add(value.data["year3"]["HK1"]["ingredientScore"]
                  [CourseName][_nameCol[i]]);
              _scoreSes2.add(value.data["year3"]["HK2"]["ingredientScore"]
                  [CourseName][_nameCol[i]]);
            }
          }
        });
              ...
              child: new IconButton(
                  ...
                  onPressed: () {
                    ...
                    Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => MyTranscriptPage()));
                  })),
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    ...
                    Text(
                      getAvg(_scoreSes1).toStringAsFixed(1).toString(),
                      style: TextStyle(
                          fontWeight: FontWeight.bold, fontSize: 20.0),
                    )
                  ],
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: <Widget>[
                    ...
                    Text(
                      getAvg(_scoreSes2).toStringAsFixed(1).toString(),
                      style: TextStyle(
                          fontWeight: FontWeight.bold, fontSize: 20.0),
                    )
                  ],
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                if (index == 0) {
                  if (scoreController != null)
                    if (scoreController.length != 0 ) {
                      scoreController.removeRange(0, scoreController.length);
                    }
                  return ListTile(
                    title: TextFormField(
                        style: _biggerFont,
                  ...
                  scoreController.add(TextEditingController(text: score[index - 1].toString()));
                  return Dismissible(
                      child: ListTile(
                        title: TextFormField(
                            style: _biggerFont,
                            controller: scoreController[index - 1],
                            decoration: InputDecoration(
                              fillColor: Colors.white,
                              hintText: '',
          ...
          actions: <Widget>[
            ...
            FlatButton(
              ...
              onPressed: () {
                ...
                double temp_number;
                if (temp != "") {
                  temp_number = double.parse(temp);

                  if (temp_number < 0) {
                    temp_number *= -1;
                    temp = temp_number.toString();
                  }
                  if (temp_number > 10) {
                    setState(() {
                      scoreError = "Hãy nhập điểm <= 10";
                    });
                    Navigator.of(context).pop();
                    _showMyDialog(nameCol, score, ses);
                  }
                    ...
                    if (temp != "") {
                      if (ses == 1) {
                        for (int i = 0; i < _nameCol.length; i++)
                          if (_nameCol[i] == nameCol) num = i;
                        List newList = List();
                        for (int i = 0; i < scoreController.length; i++) {
                          newList.add(scoreController[i].text);
                        }
                        newList.add(temp);
                        setState(() {
                          _scoreSes1[num] = newList;
